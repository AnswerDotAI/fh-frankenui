...
def MakeTaskForm():
    return Form(cls='uk-modal-body uk-modal-dialog space-y-6')(
             H3('Create Task'),P(cls=f'{TextT.muted_sm}')('Fill out the information below to create a new task',), 
             ...
             Div(cls='grid grid-cols-2')(
                UkButton(type='button', cls=UkButtonT.ghost + ' uk-modal-close')('Cancel'),
                UkButton(type='button', cls=UkButtonT.primary + ' uk-modal-close')('Submit')),)
...
@app.get('/')
def homepage():
    return Div(id='TaskForm',cls='uk-modal  p-6 uk-modal-container', uk_modal=True)(MakeTaskForm()),Div(cls='p-8')(
        ...
        Div(cls='mt-8 flex items-center justify-between')(
                 Div(cls='flex flex-1 gap-4')(
                    ...
                    UkButton('Create Task',cls='''uk-button-primary text-xs font-medium''', uk_toggle="target: #TaskForm", type='button'))),
